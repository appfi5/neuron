...
const Perun = () => {
  ...
  const [perunState, setPerunState] = useState<Subject.PerunRequest>({ type: 'SignMessage' })
  ...
  PerunRequestSubject.subscribe(requests => {
    setPerunState(requests[0])
  })
  ...
  const handleOpenChannel = async () => {
    ...
    const actionRes = await perunServiceAction({
      type: 'open',
      payload: {
        me: getParticipantByAddressAndPubkey(myAddress, myPubKey),
  ...
  const getChannels = async () => {
    ...
    const myPubKey = res.result
    console.log('requester----', getParticipantByAddressAndPubkey(myAddress, myPubKey))
    const actionRes = await perunServiceAction({
      type: 'get',
      payload: {
        requester: getParticipantByAddressAndPubkey(myAddress, myPubKey),
      },
    })
